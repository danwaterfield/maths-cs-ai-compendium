...
- If $B$ has column $\begin{bmatrix} 2 \\ 3 \end{bmatrix}$, the result column is $2 \times (\text{column 1 of } A) + 3 \times (\text{column 2 of } A)$.
...
- Multiplying the matrix by the vector row by row (each row dotted with $\begin{bmatrix} x_1 \\ x_2 \end{bmatrix}$) gives two equations:
...
- Verify: $\begin{bmatrix} 2 & 1 \\ 1 & 3 \end{bmatrix} \begin{bmatrix} 1 \\ 3 \end{bmatrix} = \begin{bmatrix} 2 + 3 \\ 1 + 9 \end{bmatrix} = \begin{bmatrix} 5 \\ 10 \end{bmatrix}$. It checks out.
